...
class ClogModuleMenuComponent extends Component {
	...
	public function menus($key,$menus) {
		...
		/*
		 * merging or create new viewVars for clog_menus
		 */
	...
	public function add($key,$newMenus) {
		...
		if(!array_key_exists($controllerName,$this->_menus)) {
			 ...
			 * reset
			*/
			if(array_key_exists('clog_menus',$controllerVars)
					&& array_key_exists($key,$controllerVars['clog_menus'])) {
				unset($this->_Controller->viewVars['clog_menus'][$key]);
			}
			
			$this->_Controller->viewVars['clog_menus'][$key] = $this->_menus[$controllerName][$key];
	...
	public function remove($key) {
		...
		if(array_key_exists('clog_menus',$controllerVars) && array_key_exists($key,$controllerVars['clog_menus'])) {
			unset($this->_Controller->viewVars['clog_menus'][$key]);
		}
	...
	private function _mergingControllerViewVars($controllerName) {
		...
		/*
		 * merge with Controller view vars inside clog_menus
		 */
		if(!empty($controllerVars) && array_key_exists('clog_menus',$controllerVars)) {
			
			$this->_Controller->viewVars['clog_menus'] = array_merge(
					$this->_Controller->viewVars['clog_menus'],$this->_menus[$controllerName]);
			
		}else{
			$this->_Controller->set('clog_menus',$this->_menus[$controllerName]);
		}
