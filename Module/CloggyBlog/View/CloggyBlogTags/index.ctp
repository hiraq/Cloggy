		...
		<?php if(!empty($tags)) : ?>
			<?php foreach($tags as $tag) :?>
				<tr>					
					...
					<td>
						<?php echo $this->Html->link($tag['CloggySubject']['subject'],
								'/'.Configure::read('Cloggy.url_prefix').'/module/cloggy_blog/cloggy_blog_tags/edit/'.$tag['CloggyNode']['id']); ?>
					</td>
					<td>
					...
					<td>
						...
						<?php echo $this->Html->link('Remove',
								'/'.Configure::read('Cloggy.url_prefix').'/module/cloggy_blog/cloggy_blog_tags/remove/'.$tag['CloggyNode']['id'],array('class' => 'post_remove')); ?>																					
					</td>
				</tr>
			<?php endforeach; ?>
		<?php else: ?>
