	...
	<thead>	
		<tr>
			<th colspan="9">Categories - <?php echo $this->Html->link('Manage',
								'/'.Configure::read('Cloggy.url_prefix').'/module/cloggy_blog/cloggy_blog_categories'); ?></th>
		</tr>	
		<tr>						
	...
	<tbody>
		<?php if(!empty($categories)) : ?>
			<?php foreach($categories as $category) :?>
				<tr>					
					<td><input type="checkbox" name="category[]" value="<?php echo $category['CloggyNode']['id']; ?>" /></td>
					<td>
						<?php echo $this->Html->link($category['CloggySubject']['subject'],
								'/'.Configure::read('Cloggy.url_prefix').'/module/cloggy_blog/cloggy_blog_categories/edit/'.$category['CloggyNode']['id']); ?>
					</td>
					<td>
					...
					<td>
						...
						<?php echo $this->Html->link('Remove',
								'/'.Configure::read('Cloggy.url_prefix').'/module/cloggy_blog/cloggy_blog_categories/remove/'.$category['CloggyNode']['id'],array('class' => 'post_remove')); ?>																					
					</td>
				</tr>
			<?php endforeach; ?>
		<?php else: ?>
