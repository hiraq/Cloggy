	...
	<tbody>
		<?php if(!empty($tags)) : ?>
			<?php foreach($tags as $tag) :?>
				<tr>					
					<td><input type="checkbox" name="tag[]" value="<?php echo $tag['ClogNode']['id']; ?>" /></td>
					<td>
	...
	jQuery('.action_js').on('click',function(e) {
		...
		switch(id) {
			...
			case 'action_delete_all':
				urlAjax = '<?php echo Router::url('/'.Configure::read('Clog.url_prefix').'/module/clog_blog/clog_blog_ajax/delete_all_tags'); ?>';
